...
│                              videos routes in views.py     analyze()         │
│                                                             export_dataset()  │
└──────────────┬───────────────────────┬────────────────────────────┬───────────┘
...
| `datasets/io/manifest.py` | `ImportManifest` | Add-mode journal written **before** the first write: spec + plan fingerprints, id namespace, per-table pre-import Lance versions, importer semver. Records exclusive-write eligibility; rollback requires unchanged post-import versions. |
...
| `cli/data.py` (rewritten) | `import`, `export`, `jobs`, `formats` | §8/§9. `cli/_schema_loader.py` retained solely behind `--importer` / `--info-py` escape hatches (CLI/Python only). |
...
v2.1 + v3.0, local dir or hub (`huggingface_hub.snapshot_download` of `meta/**` only, tag-pinned revision; **no `lerobot` dependency** — both layouts parsed with pyarrow directly, per the audit's version-drift warning). The Hub client is a standard Pixano dependency so the wizard works after a default installation or `uv sync`; no LeRobot extra is required.

**Annotation schema customization:** the GUI's Setup screen exposes annotation tools and object attributes alongside the source settings; optional record metadata and Advanced JSON are collapsed. Video tracks are included automatically for LeRobot. A partial declarative `schema` extends the source-derived schema: inferred camera views, episode metadata fields, and extracted state/action vectors remain present. User-defined record attributes must have defaults; `episode_index`, `tasks`, and `length` are reserved for source metadata. Object attributes may be required, since entities are created during annotation. The reviewed plan shows the resulting schema, and import uses the same resolution rules. A full `schema_manifest` remains an exact schema override.
...
- **Rollback (add mode).** `DELETE /io/jobs/{id}` / `pixano data jobs rollback` restores pre-import versions only when the dataset identity, table inventory, and all post-import versions match the manifest. Add imports and rollback hold the same cross-process dataset write lock as normal mutations; other writes fail promptly with a retryable conflict. Cached handles are refreshed under the lock. Any mismatch, a legacy manifest without exclusive-write evidence, or a resumed import is rejected before restoration. Namespace deletion is forbidden. Multi-table restoration is not a cross-table transaction.
...
**GUI wizard** (`ImportWizard.svelte`, with the existing 2 s job-polling pattern): **Setup → Review → background job**. Setup is one large screen with independently scrolling Source and Annotation columns on desktop, stacked on narrow screens. Source is on the left: format icon cards, folder upload or Hub source, name, and import options. Annotation is on the right: raw-media task icon cards above supported tools and object attributes, or a fixed Video task summary for LeRobot. The Source task summary's **Change** link moves focus to the task cards without resetting setup state. Attribute rows form a compact table with expandable options; optional record metadata and Advanced JSON remain collapsed initially.

Annotation tools use user-facing names. The form does not offer keypoints, relations, or classification. Mandatory slots are included automatically: `tracklet` for video and LeRobot, `message` for Visual Q&A, and `text_span` for image–text linking. Present those capabilities as quiet **Included** labels rather than disabled tool choices. These controls configure new annotation workflows without changing backend support or filtering annotation types from imported schemas.

**Review import** opens Review with the full `ImportPlan` (findings with samples, inferred schema for confirmation, counts, media-size estimate). **Edit setup** returns to the same setup state, preserving the uploaded source, options, and schema settings; edits require fresh analysis before import. **Start import** registers the accepted job with the shared import-job store and closes the wizard automatically. `ImportJobsTray`, mounted outside the wizard, owns progress, completion and error display, and cooperative cancellation; completion refreshes the Library. If the start request fails, keep Review and its plan visible, show the error, and allow retry. The wizard has no separate progress or done step.
...
- No legacy JSONL converter is shipped. Sources must use the JSONL v2 grammar.
- v1 files fed to the v2 importer fail with an explicit unsupported-format error requiring JSONL v2.
...
| **P2 — JSONL v2 + CLI** | `formats/pixano_jsonl` importer+exporter (incl. sidecars, media-only mode); `SchemaSpec` normalization (+ serializer silent-drop fix); format auto-detect; new `pixano data import/export/formats` CLI; `DatasetImporterTestCase` harness; normative format spec page in docs-astro; cookbook recipes rewritten with zero Python. | Golden round-trip (import→export→import id-equal) for image/multi-view/video-track/VQA/text-span fixtures — the §5 example lines are committed fixtures; v1 files fail requiring JSONL v2. |
...
| **P4 — jobs + API + GUI** | `io/jobs.py`; `api/routers/data_io.py` (+ deprecated alias); GUI wizard (combined Source/Annotation setup → analyze preview with findings/samples → confirm → close on acceptance, with polled progress in the global jobs tray); **video browse MVP**: record list with poster thumbnails + `<video>` playback of the episode window via the Range-enabled blob route — explicitly *without* frame-accurate annotation overlay (full player = 0.8.x, own workstream). | GUI imports a COCO folder with visible preflight report and tray progress; Edit setup preserves source and annotation settings; start-request failures remain on review for retry; jobs survive server restart as `interrupted`; a LeRobot episode's window plays in the browse view (H.264 sets; AV1 documented). |
| **P5 — resume, rollback, hardening, docs** | Flush-boundary checkpoints + `--resume`; add-mode manifest + `DELETE /io/jobs/{id}` rollback (exclusive-write lock and version-match guard; conflicts refuse rollback); cancel; S3 typed rejections; Windows swap retry; docs (spec reference, importer-author guide, AV1/media-mode limitations). | kill −9 mid-LeRobot-import → `resume` completes with zero duplicates; rollback after later GUI edits is refused without changing data; `uv run pytest` green incl. revived CLI e2e on new fixtures. |
...
2. **Clean break on JSONL v1** — alias/inference deleted, not deprecated; no legacy converter is shipped. One-time churn (the cookbook has already survived one rewrite) buys a single normative grammar and one parser.
...
7. **Add-mode rollback is manifest + exclusive write lock + guarded version restoration, not a transaction** — LanceDB has no cross-table transactions. Only imports performed under the lock and never resumed can be rolled back, and later changes cause a conflict instead of deletion.
...
This work was planned as 0.7.3 and is renumbered **0.8.0** (product-owner confirmed): it removes a shipped metadata dialect and the `--info` CLI, which a patch number would misrepresent. No JSONL v1 converter is shipped; the deprecated API alias remains for one release.
