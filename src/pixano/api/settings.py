...
class Settings(BaseSettings):
    ...
    Attributes:
        data_dir: Root data directory. If library_dir or models_dir are not provided,
            they are derived as data_dir/library and data_dir/models respectively.
    ...
    def _validate_after_model(self) -> Self:
        # Derive library_dir from data_dir if not explicitly provided
        if "library_dir" not in self.model_fields_set:
            self.library_dir = self.data_dir / "library"

        # Setup S3 library directory
        if isinstance(self.library_dir, S3Path):
            try:
                ...
                raise ValueError(
                    "ERROR: Could not register S3 library_dir.\n"
