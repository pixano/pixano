...
class TestClaim:
    def test_claims_up_to_the_batch_size(self, db: psycopg.Connection) -> None:
        _enqueue(db, 10)

        claimed = queue.claim(db, "worker-a", 4)

        assert len(claimed) == 4
        assert all(chunk.attempts == 1 for chunk in claimed)
        assert claimed[0].payload == {"item_ids": [0]}

    def test_claims_in_order(self, db: psycopg.Connection) -> None:
        """L'ordre FIFO : un job soumis avant est servi avant."""
        _enqueue(db, 10)

        claimed = queue.claim(db, "worker-a", 3)

        assert [chunk.seq for chunk in claimed] == [0, 1, 2]

    def test_an_empty_queue_returns_nothing(self, db: psycopg.Connection) -> None:
        assert queue.claim(db, "worker-a", 8) == []

    def test_a_claimed_chunk_is_no_longer_claimable(self, db: psycopg.Connection) -> None:
        _enqueue(db, 4)
        queue.claim(db, "worker-a", 4)

        assert queue.claim(db, "worker-b", 4) == []

    def test_two_workers_never_take_the_same_chunk(self, db: psycopg.Connection, postgres_url: str) -> None:
        ...
        def drain(name: str) -> None:
            try:
                with psycopg.connect(postgres_url, autocommit=True) as conn:
                    while True:
                        chunks = queue.claim(conn, name, 16)
                        if not chunks:
                            return
                        taken[name].extend(chunk.id for chunk in chunks)
                        for chunk in chunks:
                            queue.finish(conn, chunk)
            except Exception as exc:  # pragma: no cover - remonté par l'assertion
                failures.append(f"{name}: {exc!r}")

        threads = [threading.Thread(target=drain, args=(name,)) for name in taken]
...
class TestRecovery:
    def test_an_expired_lease_returns_the_chunk(self, db: psycopg.Connection) -> None:
        _enqueue(db, 3)
        queue.claim(db, "worker-a", 3)
        db.execute(
            f"UPDATE {SCHEMA_NAME}.job_chunks SET lease_until = now() - interval '1 minute' " "WHERE state = 'running'"
        )

        reclaimed, abandoned = queue.reclaim_expired(db)

        assert (reclaimed, abandoned) == (3, 0)
        assert len(queue.claim(db, "worker-b", 3)) == 3

    def test_a_chunk_that_keeps_killing_workers_is_set_aside(self, db: psycopg.Connection) -> None:
        ...
        assert queue.claim(db, "worker-b", 1) == []

    def test_a_restarted_worker_returns_its_own_chunks_at_once(self, db: psycopg.Connection) -> None:
        """Le bail finirait par les rendre ; les rendre au démarrage évite deux minutes d'attente."""
        _enqueue(db, 5)
        queue.claim(db, "worker-a", 3)

        released = queue.release_own(db, "worker-a")

        assert released == 3
        assert len(queue.claim(db, "worker-a", 5)) == 5

    def test_another_workers_chunks_are_left_alone(self, db: psycopg.Connection) -> None:
        _enqueue(db, 4)
        queue.claim(db, "worker-a", 2)

        assert queue.release_own(db, "worker-b") == 0


class TestCancellation:
    def test_reports_which_jobs_were_cancelled(self, db: psycopg.Connection) -> None:
        ...
        assert queue.cancelled_jobs(db, [running, cancelled]) == {cancelled}

    def test_a_released_chunk_goes_back_to_the_queue(self, db: psycopg.Connection) -> None:
        """Ce que fait un worker qui voit l'annulation entre deux lots."""
        _enqueue(db, 2)
        chunk = queue.claim(db, "worker-a", 1)[0]

        assert queue.release(db, chunk)
