...
The schema lives in a `dataset.yaml` file at the root of the source folder:
...
You could also add per-frame entity state (e.g. `entity_dynamic_state: { attrs: { occluded: { type: bool, default: false } } }`) — DAVIS doesn't need it beyond the mask itself.
