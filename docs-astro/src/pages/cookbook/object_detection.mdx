...
The schema lives in a `dataset.yaml` file at the root of the source folder — no Python required:
...
- `"entities"` is a list of objects, each carrying the custom `attrs` declared in `dataset.yaml`
...
- The `dataset.yaml` schema defines which tables Pixano creates — adding `mask` alongside `bbox` enables multi-type annotation and AI segmentation tools.
