...
The exported `dataset.yaml` records the option so that re-import knows to expect these fields:
...
For the importer to accept these fields, declare them in `dataset.yaml` under `schema.record.attrs`. The following exemple declares a supplementary string field named `label` as a record's attribute, that will be imported from the JSONL dataset files :
...
The importer recognizes `status`, `created_at`, `updated_at`, and `comment` as Record base fields by name and sets them directly. If the value type does not match (e.g. a string where a `datetime` is expected), the import fails with a clear validation error.
