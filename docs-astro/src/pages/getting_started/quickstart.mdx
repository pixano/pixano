...
## Step 1 — Have a data directory ready

If you followed [Installation](/pixano/getting_started/installing_pixano/), you already created
one and saw the empty Library. If not:

```bash
pixano init ./my_data
```

`./my_data` is Pixano's **data directory**: your datasets live in its `library/` subfolder, and
every other command takes this directory as its first argument.
