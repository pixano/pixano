...
def test_get_integrity_checks_from_schemas(dataset_multi_view_tracking_and_image):
    bboxes = dataset_multi_view_tracking_and_image.get_data("bboxes", limit=2)

    checks = get_integry_checks_from_schemas(bboxes, "bboxes")
...
def test_check_table_integrity_detects_missing_and_duplicate_ids(dataset_multi_view_tracking_and_image):
    bboxes = [bbox.model_copy(deep=True) for bbox in dataset_multi_view_tracking_and_image.get_data("bboxes", limit=2)]

    bboxes[0].id = ""
    bboxes[1].id = "duplicate"
    errors = check_table_integrity("bboxes", dataset_multi_view_tracking_and_image, bboxes)
    assert (IntegrityCheck.DEFINED_ID, "bboxes", "id", "", "") in errors

    bboxes = [bbox.model_copy(deep=True) for bbox in dataset_multi_view_tracking_and_image.get_data("bboxes", limit=2)]
    bboxes[0].id = "duplicate"
    bboxes[1].id = "duplicate"
    errors = check_table_integrity("bboxes", dataset_multi_view_tracking_and_image, bboxes)
    assert errors == [(IntegrityCheck.UNIQUE_ID, "bboxes", "id", "duplicate", "duplicate")]


def test_check_table_integrity_detects_invalid_foreign_keys(dataset_multi_view_tracking_and_image):
    bboxes = [bbox.model_copy(deep=True) for bbox in dataset_multi_view_tracking_and_image.get_data("bboxes", limit=1)]
    ...
    errors = check_table_integrity("bboxes", dataset_multi_view_tracking_and_image, bboxes)
    assert errors == [
        (IntegrityCheck.FK_ID, "bboxes", "entity_id", "invalid_bbox", "missing_entity"),
        (IntegrityCheck.FK_ID, "bboxes", "frame_id", "invalid_bbox", "missing_frame"),
    ]


def test_check_dataset_integrity_reports_invalid_rows(dataset_multi_view_tracking_and_image_copy):
    bbox = dataset_multi_view_tracking_and_image_copy.get_data("bboxes", limit=1)[0].model_copy(deep=True)
    bbox.id = "corrupt_bbox"
    bbox.entity_id = "missing_entity"

    dataset_multi_view_tracking_and_image_copy.open_table("bboxes").add([bbox])

    errors = check_dataset_integrity(dataset_multi_view_tracking_and_image_copy)
    assert (IntegrityCheck.FK_ID, "bboxes", "entity_id", "corrupt_bbox", "missing_entity") in errors
