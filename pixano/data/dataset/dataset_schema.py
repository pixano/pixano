...
class DatasetSchema(BaseModel):
    ...
    @staticmethod
    def validate(schema_json: dict, *args, **kwargs) -> "DatasetSchema":
        ...
        if any([key not in schema_json["schemas"].keys() for key in [TableGroup.ITEM.value, TableGroup.VIEW.value]]):
            raise ValueError("Schema should contain item and views")
        if list(schema_json["schemas"][TableGroup.ITEM.value].keys()) != [TableGroup.ITEM.value]:
            raise ValueError(f"{TableGroup.ITEM.value} group should contain only one table named {TableGroup.ITEM.value}")
