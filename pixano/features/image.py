...
from pixano.features.pixano_type import PixanoType, create_pyarrow_type
...
class Image(PixanoType, BaseModel):
        ...
        if urlparse(self.uri).scheme == "":
            ...
            if self.uri_prefix is not None:
                ...
                if parsed_uri.scheme == "":
                    raise ValueError(
                        "URI prefix is incomplete, no scheme "
                        "provided (http://, file://, ...)"
                    )
    ...
    def display(self, preview=False) -> IPyImage:
        ...
        Args:
            preview (bool, optional): True to display image preview instead of
                full image. Defaults to False.
