...
from pixano.datasets.utils.image import binary_to_url
...
class Thumbnail(BaseModel):
    ...
    preview_bytes: Optional[bytes]  # type: ignore
    uri_prefix: Optional[str]

    def __init__(
        self,
        uri: str,
        bytes: bytes = None,  # type: ignore
        preview_bytes: bytes = None,  # type: ignore
        uri_prefix: str = None,
    ):
        """Initialize Image.
    ...
    def get_bytes(self) -> bytes:  # type: ignore
    ...
    def display(self, preview=False) -> IPyImage:
        ...
        Args:
            preview (bool, optional): True to display image preview instead of full
