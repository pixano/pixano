...
import { InteractiveImageSegmenter, InteractiveImageSegmenterInput, LabeledClick, Box, SegmentationResult, LabeledPointsTensor } from "./interactive_image_segmentation";
...
export class SAM implements InteractiveImageSegmenter {
    ...
    preProcessInputs(clicks: Array<LabeledClick>, box: Box, imageWidth: number, imageHeight: number): LabeledPointsTensor {
        ...
        const pointCoordsTensor = new ort.Tensor("float32", pointCoords, [1, n + num_additionalPoints, 2]);
    ...
    async segmentImage(inputs: InteractiveImageSegmenterInput): Promise<SegmentationResult> {
