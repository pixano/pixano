...
<div class="p-4 space-y-8 flex flex-col h-full overflow-y-auto custom-scrollbar" bind:this={scrollContainer}>
  {#if messagesByNumber.length === 0}
    <div class="flex flex-col items-center justify-center py-20 text-center space-y-4 m-auto">
      ...
      <div>
        ...
        <p class="text-sm text-slate-500 max-w-[200px] mx-auto">Ask a question about the image to start the conversation.</p>
      </div>
    </div>
  {:else}
