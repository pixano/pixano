...
<script lang="ts">
  ...
  $: pendingQuestion =
    lastGroup && !isQuestionCompleted(lastGroup)
...
<div class="flex flex-col h-full bg-card overflow-hidden">
  ...
  <div class="flex-1 overflow-hidden">
    <VqaChatThread {messagesByNumber} on:answerContentChange on:generateAnswer on:deleteQuestion />
  </div>

  <VqaInputArea {pendingQuestion} on:storeQuestion on:answerContentChange on:generateAnswer />
</div>
