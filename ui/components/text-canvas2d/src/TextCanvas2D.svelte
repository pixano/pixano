...
<script lang="ts">
  ...
  function scaleView(view_name: string) {
    const viewLayer: Konva.Layer = stage?.findOne(`#${view_name}`);
    if (!viewLayer) {
      console.log("TextCanvas2D.scaleView - Error: Cannot scale");
      return;
    }
  ...
  const cacheImage = () => {
    ...
    let images = stage.find(
      (node: { attrs: { id: string } }): boolean =>
        node.attrs.id && node.attrs.id.startsWith("image-"), // node is of Node type, but the attribute is of any time, which provokes linting errors
    );
  ...
  async function handleKeyDown(event: KeyboardEvent) {
    ...
    if (event.key == "i") {
      console.log("TextCanvas2D - Infos");
