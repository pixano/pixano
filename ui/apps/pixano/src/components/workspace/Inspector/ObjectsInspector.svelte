...
<script lang="ts">
  ...
  import Thumbnail from "../canvas2d/Thumbnail.svelte";
  ...
  type ObjectThumbnail = AnnotationThumbnail;
  ...
  const thumbnailsByEntityId = $derived.by<Record<string, ObjectThumbnail | null>>(() => {
    const nextThumbnails: Record<string, ObjectThumbnail | null> = {};
