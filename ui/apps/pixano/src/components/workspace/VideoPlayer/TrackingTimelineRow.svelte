...
<div class="relative h-5 w-full rounded-sm bg-[linear-gradient(90deg,rgba(245,158,11,0.06)_0%,rgba(245,158,11,0.02)_100%)]">
  {#each segments as [start, end], segIdx (segIdx)}
    ...
    >
      <span class="absolute left-1 top-0 text-[9px] leading-4 text-white/95 font-medium select-none truncate">
  ...
  {#each keyframes as kfIndex (kfIndex)}
    ...
    <div
      class="absolute top-0.5 h-4 w-2 -translate-x-1/2"
      style="left: {kfPct}%"
    >
      <div class="mx-auto mt-1 h-2 w-2 rotate-45 border border-amber-700 bg-amber-100 shadow-sm"></div>
    </div>
  {/each}
  {#if pendingIdx !== null}
